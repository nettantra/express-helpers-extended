module.exports = function(app) {
  ...
  helpersExtended.renderValidationErrors = function (err_params, html_options) {
    ...
    if (!is.Object(err_params)) {
      return null;
    }
    var error_groups = err_params['errors'];

    if (is.String(error_groups)) {
      return html_options.blockTagStart + html_options.itemTagStart + error_groups + html_options.itemTagEnd + html_options.blockTagEnd;
    }
    var object_keys = Object.keys(error_groups)[0];
    if ((_.indexOf(object_keys, 'ValidationError') == -1) && (_.indexOf(object_keys, 'invalidAttributes') == -1)) {
      return null;
    }
    ...
    _.each(error_groups, function (errors, err_type) {
      _.each(errors, function (attribute_errors, attribute_name) {
        ...
        _.each(attribute_errors, function (error) {
          ...
          errorMessages.push(html_options.itemTagStart + errorMessage + html_options.itemTagEnd);
        });
      });
    });
    return html_options.blockTagStart + _.uniq(errorMessages).join('') + html_options.blockTagEnd;
  }
